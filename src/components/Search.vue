<template>
  <div id="search">
    <md-card>
      ...
      <md-card-actions>
        <md-input-container md-inline>
          <md-input v-model="searchquery" id="searchquery"></md-input>
          <md-icon class="md-size-1x md-accent">search</md-icon>
          <md-button class="md-raised md-primary" v-on:click="search(searchquery)">Search</md-button>
        </md-input-container>
      </md-card-actions>
    </md-card>
    <br>
    ...
    <searchListResults :recall="recall">
    </searchListResults>
          ...
          for (var r = 0; r < responseData.length; r++) {
            if (responseData[r].Title != null) {
              if (responseData[r].Title.toLowerCase().includes(searchquery.toLowerCase())) {
                ...
                console.log('not over limit  lenght is'+ that.recall.length);
                
                that.recall.push(responseData[r])
                that.hasResults = true
                that.resultCount = that.recall.length //refactor to method
              }
              else{
